    ...
    <Layout>
      <div className="w-full px-12 pt-6 pb-12">
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 w-full gap-5 md:gap-6">
          <HomeItem title="Weekly Blocked Calls" stats="-" icon={<RiCalendarCheckLine className="text-blue-600 dark:text-blue-200 h-7 w-7" />} />
          <HomeItem title="Total Blocked Calls" stats={data?.getUser.calls.length} icon={<FaRegClock className="text-blue-600 dark:text-blue-200 h-7 w-7" />} />
          <HomeItem title="New Callers (Weekly)" stats="-" increase={10} icon={<FiPhoneCall className="text-blue-600 dark:text-blue-200 h-7 w-7" />} />
          <HomeItem title="Subscription Renewal" stats="12 Dec 2022" icon={<MdOutlineSubscriptions className="text-blue-600 dark:text-blue-200 h-7 w-7" />} />
        </div>
        <CallHistoryGraph />
